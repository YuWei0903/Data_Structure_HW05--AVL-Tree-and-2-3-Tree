...
private:
    ...
    void printTreeHelper(TwoThreeNode* node, const string& prefix, bool isLeft, const string& childLabel) {
        if (node) {
            ...
            if (node->children[0]) {
                printTreeHelper(node->children[0], prefix + (isLeft ? "│   " : "    "), true, "L: ");
                printTreeHelper(node->children[1], prefix + (isLeft ? "│   " : "    "), false, "R: ");
                if (node->numKeys == 2)
                    printTreeHelper(node->children[2], prefix + (isLeft ? "│   " : "    "), false, "R: ");
            }
        }
    }
};
